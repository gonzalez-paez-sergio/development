...
/**
 * write a program that takes 2 integers N and D and a tree of exactly N vertices and n-1 edges.
 * Your program should ouput the number of vertices that are at a distance of exactly D from the
 * vertex 1
 *
 * <p>Note: the integer N adn D will be given in the firt line (each separated by a single space)
 * and the tree will be given in the next N-1 lines. Each of the next N-1 lines will contain exactly
 * 2 integers A and B (each separated by a single space), meaning that there's a bidirectional
 * connection between vertex A and vertex B
 *
 * <p>Note: A graph is a undirected acyclic graph in which any two vertices are connected by exactly
 * one path
 *
 * <p>example
 *
...
public class NumberOfVerticesAtGivenDistance {
  ...
  public NumberOfVerticesAtGivenDistance(Graph1 graph, int i) {
  ...
  private int test(Graph1 graph, int i) {
