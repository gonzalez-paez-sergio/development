...
public class Contacts {
  ...
  public static void find(String string) {
    ...
    if (intRank == 0) {
      System.out.printf(
          "counter %d, rank==0 %d, iterations %d, first element %s, current element [%s], current length [%d]\n",
    ...
    System.out.printf(
        "%d, rank!=0 %d, iterations %d, first element %s, current element [%s], current length [%d]\n",
        intCounter, intRank, intIterations, st.getFirst(), string, st.size());
    // System.out.printf("%d\n", intCounter);
    // System.out.println();
  }
